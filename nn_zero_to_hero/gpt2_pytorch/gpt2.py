...
def get_batch(split):
    ...
    ix = torch.randint(len(data) - block_size, (batch_size,)) # random SINGLE starting point for batch; output shape (batch_size,)
    x = torch.stack([data[i:i+block_size] for i in ix]) # batch, so stack
...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        losses = torch.zeros(eval_iters) # batch of losses
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        wei = wei.masked_fill(self.tril[:T, :T] == 0, float('-inf')) # (B, T, T) T*T because self* AT over sequences
...
class GPTLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.lm_head = nn.Linear(n_embd, vocab_size) # in, out
    ...
    def generate(self, idx, max_new_tokens): # logits up to context length, from last step, generate and concat
        ...
        for _ in range(max_new_tokens):
            ...
            idx_cond = idx[:, -block_size:] # can only attend up to context length
            # get the predictions
            logits, loss = self(idx_cond) # B,T,C
            # focus only on the last time step, only need last step's attention score to sample next step
